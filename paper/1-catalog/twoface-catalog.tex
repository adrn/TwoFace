...
\begin{table}[h!]
    ...
    $\sigma^2$.
    For the systemic velocity and semi-amplitude, we use a broad Gaussian prior
    that is formally inconsistent between \thejoker\ and follow-up MCMC sampling
    (see \sectionname~\ref{sec:mcmc}): in \thejoker\ we assume Gaussian priors
    with $\sigma_v$ much larger than the measurement uncertainty so they can be
    neglected ($\sigma_v \approx \infty$), whereas when running MCMC we fix
    $\sigma_v = 10^3~\kms$.}
    \label{tbl:params}
\end{table}
...
flagged as \emph{needs more prior samples}, 391 as \emph{needs MCMC}; We
continue generating samples for these stars using the methodology explained
...
For example, the companion masses can be brown dwarf-like, or at long periods,
or at high inclination, all of which will make the velocity semi-amplitude, $K$,
small for a given system.
From the radial velocity data alone, there is therefore no way to select a
complete sample of stars with and without companions.
