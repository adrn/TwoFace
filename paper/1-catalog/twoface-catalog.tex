...
From the \apogee\ \DR\ data products (the \texttt{allStar} and \texttt{allVisit}
files), we select all stars with $\geq 3$ visits that each pass a set of quality
cuts.
For each visit, we require that the following bits are not set in the
\texttt{STARFLAGS} bitmask: \texttt{TODO}.
For each star, we require that the following bits are not set in the
\texttt{ASPCAPFLAGS} bitmask: \texttt{TODO}.
