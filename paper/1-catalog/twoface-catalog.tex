...
\subsection{Confident companions with uniquely-determined orbits}
...
pass this cut.
Unlike the multi-modal stars, the posterior pdfs for these stars can be
...
s~\ref{sec:catalog-unimodal} and \ref{sec:catalog-bimodal}).
...
period samplings, again using primary masses from \citet{Ness:2015}.
...
samplings.
Upper dashed line in this panel indicates the Roche radius, computed using an approximate functional form (\citealt{Eggleton:1983})
\begin{equation}
    \frac{R}{a} = \frac{0.49\,q^{-2/3}}{0.6\,q^{-2/3} +
        \ln\left(1 + q^{-1/3}\right)} \quad .
\end{equation}
...
put the minimum companion mass below the Hydrogen-burning limit, and one system
in which $M_{2, \textrm{min}} > M_1$; We discuss these systems below
...
\end{center}
\caption{%
\textit{Left panel:} Estimated minimum companion mass and primary mass for
systems in the high-$K$, unimodal sample (black markers) and bimodal sample
(blue markers and lines).
...
$q_{\textrm{min}}$, again for systems in the high-$K$, unimodal sample
(black markers) and bimodal sample (blue markers and lines).
\label{fig:mass}
}
\end{figure}


\subsubsection{2M04582049+2232000 -- brown dwarf companion candidate}
% BD candidate

\todo{What to show?}


\subsubsection{2M20130687+2044173 -- brown dwarf companion candidate}
% BD candidate

\todo{What to show?}


\subsubsection{2M01231070+1801407 -- black hole companion candidate}
% NS/BH candidate
...
\todo{Mathieu et al. 2004}

\figurename~\ref{fig:PeK}, left panel, shows the normalized orbital period and
eccentricity for all systems in the high-$K$, unimodal sample, and, right panel,
the RV semi-amplitude.
We normalize the inferred periods by the orbital period of a companion at the
surface of the primary, $P_{\textrm{surface}}$:
...
\approx 10$.
\todo{why is this novel with a large sample of giant stars}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.7\textwidth]{ecc-dist}
\end{center}
\caption{%
TODO:
\label{fig:ecc}
...
% \subsection{Eclipsing}
%
% \todo{Stars with Kepler light curves}


\section{Discussion}

\todo{comment on short period shit at low logg, eccentricities}
...
\todo{Semi-periodic variables, etc.}
