...
\title{Binary companions of red giant stars with APOGEE DR14}
...
Even sparse or few-epoch observations are useful for detecting companions,
though such data are difficult to use for measuring physical properties of
individual systems because permitted orbital solutions can be highly degenerate.
% Aims
Using a custom-built Monte Carlo sampler (\thejoker) to deliver (often highly
multi-modal) posterior beliefs about companion orbital parameters, here we
perform a search for secondary companions of over 100,000 stars observed by the
\apogee\ survey (\DR) with $\geq 3$ epochs.
% Methods
We select stars with probable companions by \todo{how?}.
% Results
We provide (1) a catalog of \ncompanions\ companions for which the companion
properties can be confidently determined, (2) a catalog of stars that likely
have companions, but for  which more observations are needed to uniquely
determine the orbital properties, and (3) posterior samplings for all \ntotal\
...
\todo{Explain that this is the procedure, but two full runs described below in results...}
...
If just 1 posterior sample is returned after exhausting the full library of
...
MCMC.''
If more than 1 posterior sample is returned after exhausting the full library of
prior samples, and the samples lie in multiple modes of the posterior pdf, the
only way to proceed is to generate more prior samples and continue running
\thejoker: these stars are flagged ``needs more prior samples.''


\subsubsection{``Needs MCMC'': Following up \thejoker\ with MCMC}

In this case, we use the location of the returned sample, or a randomly chosen
sample from those returned to generate a small ball of initial conditions and
use standard MCMC to continue sampling until we obtain \nposterior\ samples.

We use an ensemble MCMC sampling algorithm (\citealt{Goodman:2010}) implemented
in \python\ (\citealt{Foreman-Mackey:2013}) for this step.
We run with 1024 walkers for 16384 steps, take the final walker positions, and
downsample until we have \nposterior\ samples.
\todo{We compute convergence statistics for each run and report whether the
chains appear converged or not.}


\subsubsection{``Needs more prior samples'': Continuing \thejoker\ sampling}

We generate another equal-sized library of prior samples (a total of
$2\times\nprior$ samples) and continue sampling.
\todo{We note that because ... rejection sampling procedure, have to re-do
the rejection step after computing with full library of samples...}
...
\subsection{Inferring the excess variance distribution}
...
\todo{Move math to appendix since we cite relevant papers that show how?}
...
\todo{Some highlights from this catalog.}
