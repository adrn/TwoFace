...
given a new set of hyper-parameters $\bs{\alpha}$
\begin{equation}
...
be simplified to a sum over prior value ratios of the $K$ posterior samples for
each $n$ star
\begin{equation}
    \approx \frac{\mathcal{Z}_n}{K_n} \,
      \sum_k^{K_n} \frac{p(y_{nk} \given \bs{\alpha})}{p(y_{nk} \given \bs{\alpha}_0)}
      \quad .
\end{equation}
