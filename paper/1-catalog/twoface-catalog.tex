...
We also run on the null control sample (see
...
For these control sample stars, we don't continue sampling with \thejoker\ or
...
\begin{table}
    ...
    2M00000535+1504343 & -4.457 \\
    2M00000546+6152107 & -4.388 \\
    2M00000797+6436119 & -0.774 \\
    2M00000866+7122144 & -3.313 \\
    ... & ...
    \end{tabular}
...
unimodal (\eqname~\ref{eq:unimodal}): If the samples in each respective cluster
are unimodal, we call the sampling ``bimodal.''

\figurename~\ref{fig:highK-bimodal} shows a few examples of stars that meet
this criterion.
...
% For example, \todo{top panel, time X vs. time Y, node vs. large variance}.

\tablename~\ref{fig:highK-bimodal} contains ...
...
\begin{table}
    ...
    \hline
    2M21362657-0017579 & 14.639 & 0.1171 & 30.68 \\
    2M21362657-0017579 & 2.3049 & 0.0026 & 28.69 \\
    2M03180303-0004215 & 35.553 & 0.1302 & 4.541 \\
    2M03180303-0004215 & 107.22 & 0.3192 & 8.792 \\
    ... & ... & ...
    \end{tabular}
    \caption{\todo{copy pasta}This table contains \texttt{APOGEE\_ID}'s for all \nstars\ stars
    in the parent sample used in this work, for which we have posterior
    samplings in orbital parameters.
    The other column contains the 1st percentile computed over the $\ln K$
    samples for each source, \texttt{lnK\_per\_1}:
...
\todo{We could instead select stars with log(Pmax) - log(Pmin) < 1, or some other metric to select things that have constrained periods but are multimodal}
...
\todo{also look at trends with Teff?}

\todo{Cite troup, who also noted this}
...
periods ($P<XX$~day) because short period companions would have been
common-envelope during the RGB phase.
...
\todo{eccentricity distribution w/ prior over-plotted}
...
\todo{Interesting companions w/ large M2min, WD/NS/BH companions?}

\todo{Plot: M1 vs. M2min}

\todo{Plot: M2min vs. a2sini}
