...
fiber-plugged plates with a maximum of 300 fibers per each $\approx 1.5~{\rm
deg}^2$ field of view, leading to ``pencil-beam''-like sampling of the stellar
distribution.
...
are left with \todo{XX} visits for \todo{XX} unique sources.
...
In total, this work uses \todo{XX} visits and \todo{XX} unique sources.
\label{fig:nvisits}
}
\end{figure}
...
For all \todo{XX} \apogee\ stars with $\geq 3$ good visits (see
...
\begin{enumerate}
    ...
    initial conditions and use the ensemble MCMC sampler (\citealt{Goodman:})
    implemented in \python\ (\citealt{Foreman-Mackey:}) to continue sampling
    until we obtain \nposterior\ samples.
    ...
    case, the only way forward is to generate more prior samples and continue
    running \thejoker\ for these sources to generate more posterior samples.
\end{description}

A full run of \thejoker\ on all \todo{XX} \apogee\ stars used in this work took
...
We use the per-source posterior samplings under the default prior described
above to infer the parameters of the assumed Gaussian distribution over the
log-jitter: $(\mu_y, \sigma_y)$.
...
companion property distributions, and is also a test of the visit velocity
uncertainties reported in the \apogee\ data products.
If the catalog uncertainties are significantly underestimated, we expect the
inferred log-jitter distribution parameters to tend towards larger values.
...
\todo{Notation relative to above...}
