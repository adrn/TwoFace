...
\tablename~\ref{tbl:highK} contains a list of all \texttt{APOGEE\_ID}'s for
stars in the high-$K$ sample.
...
panel); this result is explored in more detail in companion work
(\citealt{Circularization:2018}).
...
\begin{table}[ht]
    \centering
    \begin{tabular}{c}
    \multicolumn{1}{c}{\textbf{High-$K$ stars}}\\
    \hline
    \texttt{APOGEE\_ID}\\
    \hline
    2M00000222+5625359 \\
    ...
    \end{tabular}
    \caption{This table contains \texttt{APOGEE\_ID}'s for \nhighK\ stars
    in the ``high-$K$'' sample (see \sectionname~\ref{sec:catalog-confident}).
    }
    \label{tbl:highK}
\end{table}
