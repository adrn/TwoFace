...
Less is known about uninteracting or detached companions to evolved stars.
...
\subsection{Initial individual-system posterior samplings}
...
See \tablename~\ref{tbl:params} for a description of all parameters and priors
used.
...
\begin{enumerate}
    ...
    pdf, the posterior pdf over orbital parameters is treated as effectively
    unimodal.
    In this case, we use the location of the returned sample, or the median over
    returned samples to generate a small ball of initial conditions and use
    standard MCMC to continue sampling until we obtain \nposterior\ samples.
    In detail, we use an ensemble sampler (\citealt{Goodman:2010}) implemented
    in \python\ (\citealt{Foreman-Mackey:2013}) for this step.
    We run with 1024 walkers for 16384 steps, take the final walker positions,
    and downsample until we have \nposterior\ samples.
    We compute convergence statistics for each run and report whether the chains
    appear converged or not.
...
\todo{XX} stars were flagged as \emph{needs MCMC}, \todo{YY} were flagged as
\emph{needs more prior samples}, and the remaining \todo{ZZ} stars completed normally.
...
over the log-excess-variance parameter: $(\mu_y, \sigma_y)$.
