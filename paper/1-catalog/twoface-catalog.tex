...
the same parameters.
\todo{summary of this run - do any stars get flagged as needs MCMC/etc?}
...
for a given system.
From the radial velocity data alone, there is therefore no way to define a
simple cut to select a complete sample of stars with companions.
Nevertheless, it is possible to define a cut that selects stars with
...
$K$.
\figurename~\ref{fig:lnK-percentiles}, left panel, shows distributions of
percentiles in $\ln K$ computed for all stars with posterior samplings from
\thejoker.
\figurename~\ref{fig:lnK-percentiles}, right panel, shows the same for posterior
samplings for the null control sample.
We define a cut in the \todo{XX} percentile of the $\ln K$ samples such that
\todo{XX false-positives...} (vertical line in
\figurename~\ref{fig:lnK-percentiles}); \nmultimodal\ stars pass this cut.
...
% % Notebook: "TODO"
% Should also include period-eccentricity panels with samples
% \begin{figure}[h]
% \begin{center}
% \includegraphics[width=\textwidth]{highK}
% \end{center}
% \caption{%
% TODO
% \label{fig:highK}
...
% \begin{figure}[h]
% \begin{center}
% \includegraphics[width=\textwidth]{lowK}
% \end{center}
% \caption{%
% TODO
% \label{fig:lowK}
...
\todo{\emph{no multiplets} punt to future work}

\todo{\emph{Kepler} evidence for coherent oscillations at the upper giant branch? companions that don't make sense}

\todo{\emph{SB1} comment that there are some companions with masses large enough that they should be evolved, and therefore should contribute to the light! where are the companions? neutron stars / BHs?}
