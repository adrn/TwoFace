...
\todo{How do we actually select these?}
...
criteria.
These stars are prime candidates for follow-up efforts.
In many of these cases, there are certain times at which a future observation
would be far more informative.
Visually, these times correspond to regions of time when the predicted RVs have
large variance as computed for orbits computed from the posterior samplings.
For example, \todo{top panel, time X vs. time Y, node vs. large variance}.

\tablename~\ref{tbl:todo} contains ...
...
star.
We consider a sampling to be unimodal when $P_{\textrm{max}} - P_{\textrm{min}}
< \Delta$ (see \sectionname~2 of \citealt{Price-Whelan:2017}); \nunimodal\ stars
pass this cut.
