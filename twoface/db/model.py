...
class StarResult(Base):
    ...
    jokerrun_id = Column('jokerrun_id', types.Integer, ForeignKey('jokerrun.id'))
    status_id = Column('status_id', types.Integer, ForeignKey('status.id'),
                       default=0)
    ...
    def __repr__(self):
        return ("<JokerResult(apogee_id='{}', joker_run='{}', status='{}')>"
                .format(self.star.apogee_id, self.jokerrun.name, self.status.message))
...
class AllStar(Base):
    ...
    id = Column(types.Integer, primary_key=True)
    visits = relationship("AllVisit", secondary=AllVisitToAllStar, back_populates="stars")

    joker_runs = relationship("JokerRun", secondary="starresult", back_populates="stars")
    results = relationship("StarResult")

    # Columns from the FITS file, auto-generated by scripts/_generate_model_columns.py
    ...
    def __repr__(self):
        return "<ApogeeStar(id='{}', apogee_id='{}', {} results)>".format(self.id,
                                                                          self.apogee_id,
                                                                          len(self.results))
...
class AllVisit(Base):
    ...
    id = Column(types.Integer, primary_key=True)
    stars = relationship("AllStar", secondary=AllVisitToAllStar, back_populates="visits")

    # Columns from the FITS file, auto-generated by scripts/_generate_model_columns.py
    visit_id = Column("visit_id", types.String)
    ...
    k = Column("k", types.REAL)
    k_err = Column("k_err", types.REAL)
    ra_targ = Column("ra_targ", types.Numeric)
    dec_targ = Column("dec_targ", types.Numeric)
    apogee_target1 = Column("apogee_target1", types.Integer)
    apogee_target2 = Column("apogee_target2", types.Integer)
    apogee_target3 = Column("apogee_target3", types.Integer)
    targflags = Column("targflags", types.String)
...
class JokerRun(Base):
    ...
    prior_samples_file = Column('prior_samples_file', types.String, nullable=False)

    stars = relationship("AllStar", secondary="starresult", back_populates="joker_runs")
    ...
    requested_samples_per_star = Column('requested_samples_per_star', types.Integer, nullable=False)
    max_prior_samples = Column('max_prior_samples', types.Integer, nullable=False)

    def __repr__(self):
        return "<JokerRun(name='{}', date='{}')>".format(self.name, self.date.isoformat())
